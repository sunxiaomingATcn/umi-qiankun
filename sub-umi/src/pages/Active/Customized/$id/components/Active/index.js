...
class Active extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (JSON.stringify(nextProps.activeV3) !== JSON.stringify(prevState.activeV3)) {
            const { activeV3, activeV3: { discount: { remainingNumber, countDownTime } }, activeParams } = nextProps;
            return {
                activeV3,
                remainingNumber,
                activeParams,
    ...
    collectBaiduHmView = () => {
        ...
        Utils.collectBaiduHm(`1v1_v3.0_${amount}_${channel}_在线定制保险方案`, "open")
        const scrollCollectDomArr = ["view1", "view2", "view3", "view4", "view5", "view6", "view7", "view8"]
        scrollCollectDomArr.forEach(item => {
            Utils.domTAndBView(() => {
                Utils.collectBaiduHm(`1v1_v3.0_${amount}_${channel}_在线定制保险方案`, item)
            }, item)
        })
    }
    ...
    goTop = () => {
        const { amount, channel } = this.state;
        Utils.collectBaiduHm(`1v1_v3.0_${amount}_${channel}_在线定制保险方案`, 'click')
        Utils.goTop();
    }
