...
class Birth extends Component {
    ...
    componentDidMount() {
        const { customized: { birthday, spouseBirthday, firstBirthday, secondBirthday, thirdBirthday, fatherBirthday, motherBirthday }
        } = this.props;
        const { amount, channel } = this.props;
        Utils.collectBaiduHm(`1v1_${amount}_${channel}_表单生日`, "open")
        this.setState({
    ...
    initTabs = () => {
        const { customized: { childCount }, customized } = this.props;
        ...
        const mother = { name: "motherBirthday", value: "母亲" + (customized.motherBirthday ? "" : customized.birthday ? "(已跳过)" : "(可选)") }
        const father = { name: "fatherBirthday", value: "父亲" + (customized.fatherBirthday ? "" : customized.birthday ? "(已跳过)" : "(可选)") }
        ...
        tabs = tabs.map(i => {
            return { ...i, select: !!customized[i.name] }
        })
    ...
    */
    ...
    singleFamily = () => {
        //单(身)亲、非单(身)亲
        const { customized: { code_family_composition: familyComposition, code_singleFamily } } = this.props;
    ...
    pre = () => {
        const { prePage, customized: { code_family_composition, code_hasChilds } } = this.props;
        if (prePage) {
            prePage()
            //单身
    ...
    next = () => {
        const { nextPage, dispatch, customized: { childCount } } = this.props;
        ...
        return (
            <div className={styles.birthBox}>
                ...
                <div className={styles.footer}>
                    <button className={styles.back} onClick={() => this.prePerson()}>返回</button>
