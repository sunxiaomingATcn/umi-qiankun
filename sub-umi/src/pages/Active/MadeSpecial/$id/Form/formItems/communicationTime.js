...
@connect(({ customized, activeCustom }) => ({
    customized,
    activeCustom
}))
...
class CommunicationDate extends Component {
    ...
    componentDidMount() {
        ...
        Utils.collectBaiduHm(`1v1_${amount}_${channel}_表单预约时间`, "open")

        dispatch({ type: 'activeCustom/queryCustomizationDate' }).then(() => {
            this.initTimesList()
        })
    }

    initTimesList = (date, cb) => {
        const { activeCustom: { customizationDate = {} } } = this.props;
        let defaultDay = date;
        // 初始化
        if (!date) {
    ...
    */
    ...
    getFormatValue = () => {
        ...
        let targetKeys = Object.keys(customized).filter(i => !i.includes("_"))
        targetKeys = targetKeys.reduce((pre, next) => {
            return {
                ...pre,
                [next]: next.toLocaleLowerCase().includes("gender") ? sexCodeToStr(customized[next]) : customized[next]
            }
        }, {})
    ...
    next = () => {
        const { dispatch, formResLink, channel } = this.props;

        //提交表单
        const targetKeys = this.getFormatValue()
        Toast.loading('Loading...', 0)
        const crmUserKey = this.props.location.query.crmUserKey;
        const type = Utils.isWeiXin() && !crmUserKey ? 'activeCustom/submitCustomized' : 'activeCustom/submitUnUserCustomized';
        //console.log("submit", { ...targetKeys, channel },type)
        if (crmUserKey) {
            targetKeys.crmUserKey = crmUserKey

        }else if(!Utils.isWeiXin() && localStorage.getItem("crmUserKey")){
            targetKeys.crmUserKey = localStorage.getItem("crmUserKey")
        }
        console.log({ ...targetKeys, channel, sourceUrl: window.location.href })
        // return;
        dispatch({
            type,
